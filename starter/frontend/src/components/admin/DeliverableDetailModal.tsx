import type { CatalogDeliverable } from '../../store/catalogStore';

type DeliverableDetailModalProps = {
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-[#bec9c8] px-6 py-4 shrink-0">
          <div className="min-w-0">
            ...
            >
              {displayValue(deliverable.listado_de_entregables)}
            </h3>
            <p className="mt-0.5 text-sm text-[#3f4949] font-mono">
              {displayValue(deliverable.codigo_entregable)}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5">
          <section className="rounded-xl border border-[#E2E8F0] bg-[#f8faf9]/80 p-4 sm:p-5">
            ...
            <dl className="grid grid-cols-1 gap-y-4">
              <Field label="Código entregable" value={deliverable.codigo_entregable} />
              <Field
                label="Listado de entregables"
                value={deliverable.listado_de_entregables}
                multiline
              />
            </dl>
          </section>
